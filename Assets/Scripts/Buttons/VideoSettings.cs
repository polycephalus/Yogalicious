using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.Video;
using UnityEngine.UI;

public class VideoSettings : MonoBehaviour {
    
    public Text currentMinutes;
    public Text currentSeconds;
    public Text totalMinutes;
    public Text totalSeconds;

    private VideoPlayer videoPlayer;
    private int videoClipIndex = 0;

    //Fredrik går lös
    public bool isExercise;
    public ListHandler lh;
    private Program prog;
    private Exercise exer;
    private VideoClip[] videoClips;
    private VideoClip videoClip;

    void Awake()
    {
        if (isExercise)
        {
            exer = lh.GetComponent<ListHandler>().getSelectedExercise();
            videoClip = exer.getVideoClip();
        }
        else
        {
            prog = lh.GetComponent<ListHandler>().getSelectedProgram();
            videoClips = prog.getVideoClips();
        }
        videoPlayer = GetComponent<VideoPlayer>();
    }

    void Start()
    {
        if (isExercise)
        {
            videoPlayer.clip = videoClip;
        }
        else
        {
            videoPlayer.clip = videoClips[0];
        }
        SetTotalTimeUI();
    }
    void Update()
    {
        if (videoPlayer.isPlaying)
        {
            SetCurrentTimeUI();
        }

    }

    public void SetNextClip()
    {
        if(videoClipIndex < videoClips.Length - 1)
        {
            videoClipIndex++;
            videoClipIndex = videoClipIndex % videoClips.Length;

            videoPlayer.clip = videoClips[videoClipIndex];
            SetTotalTimeUI();
            videoPlayer.Play();
        }
    }
    public void SetFormerClip()
    {
        if(videoClipIndex > 0)
        {
            videoClipIndex--;

            videoPlayer.clip = videoClips[videoClipIndex];
            SetTotalTimeUI();
            videoPlayer.Play();
        }           
    }

    void SetCurrentTimeUI()
    {
        string minutes = Mathf.Floor((int)videoPlayer.time / 60).ToString("00");
        string seconds = ((int)videoPlayer.time % 60).ToString("00");

        currentMinutes.text = minutes;
        currentSeconds.text = seconds;
    }

    void SetTotalTimeUI()
    {
        string minutes = Mathf.Floor((int)videoPlayer.clip.length / 60).ToString("00");
        string seconds = ((int)videoPlayer.clip.length % 60).ToString("00");

        totalMinutes.text = minutes;
        totalSeconds.text = seconds;
    }
}
